export default class Header {
  ...
  _createSaved(isAuthorized, name) {
    this._clear();
    return document.createRange().createContextualFragment(
      isAuthorized
        ? `
        ...
        <div class="header__container">
            ...
            <button class="header__logout header__logout_theme_white">${name}</button>
        </div>
        ` : `
        <p class="header__logo header__logo_theme_white">NewsExplorer</p>
        <div class="header__container">
  ...
  render(parameters) {
    ...
    isSaved ? this._container.appendChild(this._createSaved(isAuthorized, name)) : this._container.appendChild(this._createBase(isAuthorized, name));
  }
}
